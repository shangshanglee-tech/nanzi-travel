function buildVesselDetailState(vessel = {}) {
  ...
  if (vessel.has_science_center) facilities.push({icon: "science-center", text: "科研中心"});
  if (vessel.has_wifi) facilities.push({icon: "wifi", text: "免费 Wi-Fi"});
  if (vessel.is_hybrid) facilities.push({icon: "hybrid", text: "环保混合动力引擎"});
  if (vessel.has_stabilization_system) facilities.push({icon: "modern-stable-tech", text: "船身稳定技术"});
  ...
  if (vessel.has_infinity_pool) facilities.push({icon: "swimming-pool", text: "无边泳池"});
  if (vessel.heated_pool_count) facilities.push({icon: "hot-tubs", text: `${vessel.heated_pool_count} 个恒温泳池`});
