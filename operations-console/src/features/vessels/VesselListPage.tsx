...
export function VesselListPage() {
  ...
  return <Card title="船只"><Table rowKey="id" loading={loading} dataSource={items} pagination={false} columns={[
    { title: "卡片图", width: 100, render: (_, item: Vessel) => item.card_image ? <Image width={64} height={42} preview={false} style={{ objectFit: "cover" }} src={item.card_image} alt={item.name} /> : "未设置" },
