import { Button, Card, Checkbox, Col, Form, Image, Input, InputNumber, Row, Select, Space, Spin, Tabs, Upload, message } from "antd";
...
export function VesselEditorPage() {
  const { id } = useParams(); const navigate = useNavigate(); const [form] = Form.useForm<VesselInput>(); const [vessel, setVessel] = useState<Vessel | null>(null); const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false); const [messageApi, holder] = message.useMessage();
  async function load() { if (!id) return; setLoading(true); try { const value = await getVessel(id); setVessel(value); form.setFieldsValue(value); } catch { messageApi.error("加载船只资料失败"); } finally { setLoading(false); } }
  useEffect(() => { void load(); }, [id]);
  async function save(values: VesselInput) { if (!vessel) return; setSaving(true); try { await updateVessel(vessel.id, values); await load(); messageApi.success("已保存"); } catch { messageApi.error("保存失败，请检查发布状态与卡片图"); } finally { setSaving(false); } }
  async function uploadCard(file: File) { if (!vessel) return; try { await uploadVesselCardImage(vessel.id, file); await load(); messageApi.success("卡片图已更新"); } catch { messageApi.error("卡片图上传失败"); } }
  if (loading || !vessel) return <Spin />;
  return <>{holder}<Space style={{ marginBottom: 16 }}><Button onClick={() => navigate("/vessels")}>返回列表</Button><Button type="primary" loading={saving} onClick={() => form.submit()}>保存</Button></Space><Form form={form} layout="vertical" onFinish={(values) => void save(values)}><Tabs items={[
    ...
    { key: "card", label: "首页卡片", children: <Card><Form.Item label="卡片底色" name="card_tone" rules={[{ pattern: /^#[0-9A-Fa-f]{6}$/, message: "请输入 #071A32 格式色值" }]}><Input placeholder="#071A32" /></Form.Item>{vessel.card_image ? <Image width={280} src={vessel.card_image} alt="船只卡片图" /> : <p>暂未设置卡片图。</p>}<div style={{ marginTop: 16 }}><Upload accept="image/*" showUploadList={false} beforeUpload={selectOnly} onChange={({ file }) => file.originFileObj && void uploadCard(file.originFileObj)}><Button>上传或替换卡片图</Button></Upload></div></Card> },
